...
public class playerAnimation : MonoBehaviour
{
    ...
    {
        //Essentials
        var dt = Time.deltaTime; //set the delta time variable
        ...
        if (yInput > 0 && _animTime < 0.5f) //if input = up, and ship is not tilted all the way up...
        { _animTime += _timeSpeed * dt; } //...then tilt ship up

        if (yInput == 0 && _animTime > 0.3f) //if there is no input, and ship is tilted up...
        { _animTime -= _timeSpeed * dt; } //...then tilt ship back down

        if (yInput < 0 && _animTime > 0) //if input = down, and ship is not tilted all the way down...
        { _animTime -= _timeSpeed * dt; } //...then tilt ship down

        if (yInput == 0 && _animTime < 0.2f) //if there is no input, and ship is tilted down...
        { _animTime += _timeSpeed * dt; } //...then tilt ship back up


        ////////////////////////////////////////////////////////////////////////////////////////////////////
        //apply tilt angles
        transform.eulerAngles = new Vector3(_tilt, 0, 0);
    }
}
